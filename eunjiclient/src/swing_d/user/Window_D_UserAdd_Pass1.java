...
public class Window_D_UserAdd_Pass1 extends JPasswordField{
	int x;
	int y;
	int width;
	int height;
	...
	public Window_D_UserAdd_Pass1(int x, int y, int location_x , int location_y, int width , int height){
		...
		addKeyListener(new KeyAdapter() {
			public void keyReleased(KeyEvent e) {
				...
				if(getText().length()==0) {
					key = true;
					pass = false;
					Window_interface_D.User_Pass_labeL1.a = "패스워드를 입력해 주세요";
					Window_interface_D.User_Pass_labeL1.repaint();
					...
					key = true;
					pass = false;
					Window_interface_D.User_Pass_labeL1.a = "패스워드 길이는 8 ~ 16자로 입력해주세요";
					Window_interface_D.User_Pass_labeL1.repaint();
				}
				else {
					key = false;
					pass = true;
					Window_interface_D.User_Pass_labeL1.a = "2차 패스워드 완료";
					Window_interface_D.User_Pass_labeL1.repaint();
